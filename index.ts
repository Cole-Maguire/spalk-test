...
function parsePacket(buffer: Buffer, packet: number): number {
	...
	if(syncByte !== SYNC_BYTE) {
		throw new Error(`Error: No sync byte present in packet ${packet}, offset ${packet*PACKET_SIZE}`);
	}
	console.debug(buffer.at(1).toString(16),FLAG_MASK.toString(16), buffer.at(2).toString(16));
